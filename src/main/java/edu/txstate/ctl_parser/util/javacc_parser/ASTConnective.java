...
public class ASTConnective extends CTLFormulaNode {
        ...
        switch (type) {
            case EX:
                for (State state : model.getStates().values()) {
                    if (checkE(state, (CTLFormulaNode) jjtGetChild(0)) && !state.isMarked(marking)) {
                        state.mark(marking);
                    }
                }
                break;
            case EU:
                ...
                while (changed) {
                    changed = false;
                    for (State state : model.getStates().values()) {
                        if ((checkE(state, this) && ((CTLFormulaNode) jjtGetChild(0)).check(state))
            ...
            case AF:
                for (State state : model.getStates().values()) {
                    if (checkA(state, (CTLFormulaNode) jjtGetChild(0)) ||
                ...
                while (changed) {
                    changed = false;
                    for (State state : model.getStates().values()) {
                        if (checkA(state, this) && !state.isMarked(marking)) {
                            System.out.println(state.getName() + ": " + checkA(state, this) + ", " + ((CTLFormulaNode) jjtGetChild(0)).check(state));
                            state.mark(marking);
                            changed = true;
                        }
                    }
    ...
    private boolean checkE(State state, CTLFormulaNode phi) {
    ...
    private boolean checkA(State state, CTLFormulaNode phi) {
